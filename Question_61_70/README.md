...
## Q.66. HOG (Step.1) 梯度幅值・梯度方向

*imoir.jpg*のHOG特徴量の梯度幅值・梯度方向を求めよ。
...
```
2. gx, gyから梯度幅值と梯度方向を求める。

```bash
梯度幅值: mag = sqrt(gt ** 2 + gy ** 2)
梯度方向: ang = arctan(gy / gx)
```
3. 梯度方向を [0, 180]で9分割した値に量子化する。つまり、[0,20]には0、[20, 40]には1というインデックスを求める。
4. 画像をN x Nの領域に分割し(この領域をセルという)、セル内で3で求めたインデックスのヒストグラムを作成する。ただし、当表示は1でなく梯度方向を求める。
...
N=8として、8x8の領域を1セルとして、梯度方向のインデックスに梯度幅值を投票する形式でヒストグラムを作成せよ。
...
つまり、青色のカラートラッキングを行うにはHSV変換を行い、180<=H<=260となる位置が255となるような二値画像を输出すればよい。
